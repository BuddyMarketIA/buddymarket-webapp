...
function heatColor(value: number, max: number): string {
  if (max === 0) return "bg-muted/50 text-muted-foreground/70";
  const ratio = value / max;
  if (ratio === 0) return "bg-muted/50 text-muted-foreground/70";
...
function categoryColor(cat: string): string {
  ...
  return map[cat] ?? "bg-muted/50 text-muted-foreground";
}
...
export default function UsageAnalyticsPanel() {
  ...
  if (loadingOverview) {
    return (
      <div className="space-y-3">
        {[1, 2, 3].map(i => (
          <div key={i} className="vively-card animate-pulse h-24 bg-muted/30" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      {/* ── KPI Cards ── */}
      <div>
        <h3 className="mb-2 flex items-center gap-1.5 text-sm font-bold text-foreground/80">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{overview?.users.total ?? 0}</p>
          ...
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{aiTotal}</p>
            <p className="text-xs text-muted-foreground">Acciones con IA</p>
          ...
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{overview?.menus.total ?? 0}</p>
          ...
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{overview?.shoppingLists.total ?? 0}</p>
            <p className="text-xs text-muted-foreground">Listas de compra</p>
          ...
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{overview?.mealLogs.aiPhoto ?? 0}</p>
            <p className="text-xs text-muted-foreground">Análisis IA foto</p>
          ...
          <div className="vively-card text-center">
            ...
            <p className="text-2xl font-extrabold text-foreground">{overview?.mealLogs.total ?? 0}</p>
            <p className="text-xs text-muted-foreground">Registros diario</p>
      ...
      <div className="vively-card space-y-3">
        ...
        {loadingAI ? (
          <div className="animate-pulse h-16 bg-muted/30 rounded-xl" />
        ) : (
              ...
              return (
                <div key={item.label}>
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <span className="text-xs font-bold text-foreground">{item.value} <span className="text-muted-foreground/70 font-normal">({pct}%)</span></span>
                  </div>
      ...
      <div className="vively-card space-y-3">
        ...
        <p className="text-xs text-muted-foreground/70">Uso acumulado por funcionalidad. El color indica la intensidad de uso relativa.</p>
        {loadingHeatmap ? (
      ...
      <div className="vively-card space-y-3">
        ...
        <p className="text-xs text-muted-foreground/70">Clasificados por número de registros en el diario alimentario.</p>
        {loadingTop ? (
          <div className="animate-pulse h-24 bg-muted/30 rounded-xl" />
        ) : (
          <div className="space-y-1.5">
            {(topUsers ?? []).map((u, i) => (
              <div key={u.userId} className="flex items-center gap-3 rounded-xl bg-muted/30 px-3 py-2">
                <span className={`shrink-0 flex h-6 w-6 items-center justify-center rounded-full text-xs font-extrabold ${
                  i === 0 ? "bg-yellow-400 text-white" :
                  i === 1 ? "bg-gray-300 text-foreground/80" :
            ...
            {(!topUsers || topUsers.length === 0) && (
              <p className="text-center text-xs text-muted-foreground/70 py-4">Sin datos de actividad todavía.</p>
            )}
          </div>
        )}
      </div>
      ...
      <div className="vively-card">
        <h3 className="mb-2 text-sm font-bold text-foreground/80">Otras métricas</h3>
        <div className="grid grid-cols-2 gap-2">
          {[
            ...
            <div key={stat.label} className="flex items-center gap-2 rounded-xl bg-muted/30 px-3 py-2">
              <span className="text-lg">{stat.icon}</span>
              <div>
                <p className="text-sm font-extrabold text-foreground">{stat.value.toLocaleString("es-ES")}</p>
