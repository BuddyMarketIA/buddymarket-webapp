        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {plan.weekNumber && (
                <p className="text-xs text-gray-400 mt-0.5">Semana {plan.weekNumber} · {plan.year}</p>
              )}
            </div>
          </div>
        ...
        <div className="p-5">
          {!showPrefs ? (
            <div className="text-center py-4">
              ...
              <p className="text-gray-600 mb-4 text-sm">Tu experto ha preparado un plan personalizado. Genera tu menú semanal y lista de la compra con IA.</p>
              <Button
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="font-semibold text-gray-800">Personaliza tu menú <span className="text-gray-400 font-normal text-sm">(todos los campos son opcionales)</span></h4>
                <Button variant="ghost" size="sm" onClick={() => setShowPrefs(false)}>Cancelar</Button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <Label className="text-xs text-gray-600">Alergias o intolerancias</Label>
                  <Input placeholder="Ej: lactosa, gluten..." value={allergies} onChange={e => setAllergies(e.target.value)} className="mt-1 text-sm" />
                </div>
                <div>
                  <Label className="text-xs text-gray-600">Restricciones dietéticas</Label>
                ...
                <div>
                  <Label className="text-xs text-gray-600">Alimentos que no te gustan</Label>
                ...
                <div>
                  <Label className="text-xs text-gray-600">Tiempo disponible para cocinar</Label>
                ...
                <div>
                  <Label className="text-xs text-gray-600">Número de personas</Label>
              ...
              <p className="text-xs text-gray-400 text-center">La generación puede tardar entre 15 y 30 segundos.</p>
            </div>
          )}
        </div>
      )}
        ...
        <div className="p-5">
          ...
          <div className="flex gap-2 mb-5">
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors ${activeTab === "menu" ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
            >
              🗓️ Menú semanal
            </button>
            <button
              onClick={() => setActiveTab("shopping")}
              className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors ${activeTab === "shopping" ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}
            >
          ...
          {activeTab === "menu" && (
            <div className="space-y-4">
              {DAYS.map((day) => {
                ...
                return (
                  <div key={day} className="bg-orange-50 rounded-xl p-4 border border-orange-100">
          ...
          {activeTab === "shopping" && shoppingList && (
            <div className="space-y-4">
              {Object.entries(SHOPPING_LABELS).map(([key, label]) => {
                ...
                return (
                  <div key={key}>
                    <h4 className="font-semibold text-gray-700 mb-2 text-sm">{label}</h4>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
                      {items.map((item, i) => (
                        <div key={i} className="flex items-center gap-1.5 bg-gray-50 rounded-lg px-3 py-1.5 text-sm text-gray-700">
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Planes personalizados asignados por tu BuddyExpert. Genera tu menú semanal y lista de la compra con un clic.</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-orange-50 rounded-2xl border-2 border-dashed border-orange-200">
          ...
          <h3 className="text-lg font-semibold text-gray-700 mb-2">Aún no tienes planes asignados</h3>
          <p className="text-gray-500 mb-6">Cuando tu BuddyExpert te asigne un plan personalizado, aparecerá aquí.</p>
          <Link href="/experts">
