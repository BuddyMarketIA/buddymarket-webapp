...
function StatusBadge({ status }: { status: string }) {
  ...
  const { label, className } = map[status] ?? { label: status, className: "bg-muted/50 text-muted-foreground" };
  return (
      ...
      <div className="mx-auto max-w-lg px-4 -mt-6 space-y-4">
        ...
        <div className="rounded-3xl bg-background shadow-lg overflow-hidden">
          <div className="p-5">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-foreground/80">Tu código de referido</span>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              <button
                onClick={handleShare}
                className="flex flex-col items-center gap-1.5 rounded-2xl border border-border py-3 text-xs font-semibold text-foreground/80 hover:bg-muted/30 active:scale-95 transition-transform"
              >
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-2xl bg-background p-4 text-center shadow-sm">
            ...
            <p className="mt-0.5 text-xs text-muted-foreground">Invitados</p>
          </div>
          <div className="rounded-2xl bg-background p-4 text-center shadow-sm">
            <p className="text-2xl font-black text-green-600">{rewardedCount + subscribedCount}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">Suscritos</p>
          </div>
          <div className="rounded-2xl bg-background p-4 text-center shadow-sm">
            <p className="text-2xl font-black text-purple-600">{stats?.totalRewardDays ?? 0}</p>
            <p className="mt-0.5 text-xs text-muted-foreground">Días gratis</p>
          </div>
        </div>

        {/* How it works */}
        <div className="rounded-3xl bg-background p-5 shadow-sm">
          <h3 className="mb-4 text-sm font-bold text-foreground">¿Cómo funciona?</h3>
        ...
        <div className="rounded-3xl bg-background p-5 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-foreground">¿Tienes un código de amigo?</h3>
              <p className="text-xs text-muted-foreground mt-0.5">Aplícalo para que tu amigo gane su recompensa</p>
            </div>
            <button
          ...
          {showApplyCode && (
            <div className="mt-4 flex gap-2">
              <input
                ...
                maxLength={8}
                className="flex-1 rounded-2xl border border-border px-4 py-2.5 text-center text-lg font-bold tracking-widest uppercase outline-none focus:border-[#F97316] focus:ring-1 focus:ring-[#F97316]"
              />
              <button
                onClick={() => {
        ...
        {(stats?.referrals ?? []).length > 0 && (
          <div className="rounded-3xl bg-background p-5 shadow-sm">
            <h3 className="mb-4 text-sm font-bold text-foreground">Tus referidos</h3>
            <div className="space-y-3">
              {(stats?.referrals ?? []).map((referral) => (
                <div key={referral.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground truncate">{referral.referredName}</p>
        ...
        {(stats?.referrals ?? []).length === 0 && (
          <div className="rounded-3xl bg-background p-8 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm font-semibold text-foreground/80">Aún no has invitado a nadie</p>
            <p className="mt-1 text-xs text-muted-foreground/70">Comparte tu código y empieza a ganar meses gratis</p>
            <button
              onClick={handleShare}
