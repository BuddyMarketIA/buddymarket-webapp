...
export const wearablesRouter = router({
  ...
  /**
   * Get OAuth authorization URL for Oura Ring
   */
  getOuraAuthUrl: protectedProcedure
    ...
    .query(({ input }) => {
      ...
      return {
        url: `https://cloud.ouraring.com/oauth/authorize?${params.toString()}`,
      };
    }),

  /**
   * Exchange Oura authorization code for access token
   */
  connectOura: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!clientId || !clientSecret) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Oura not configured" });
      }
      ...
      if (!tokenResponse.ok) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Failed to exchange code for token" });
      }
  ...
  /**
   * Get OAuth authorization URL for Whoop
   */
  getWhoopAuthUrl: protectedProcedure
    ...
    .query(({ input }) => {
      ...
      const params = new URLSearchParams({
        ...
        redirect_uri: input.redirectUri,
        response_type: "code",
        scope: "read:cycles read:recovery read:sleep read:profile",
      });
  ...
  /**
   * Exchange Whoop authorization code for access token
   */
  connectWhoop: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!clientId || !clientSecret) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Whoop not configured" });
      }
  ...
  /**
   * Get all connected wearables for current user
   */
  ...
  disconnect: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await drizzleDb
        ...
        .where(and(eq(wearableConnections.userId, ctx.user.id), eq(wearableConnections.wearableType, input.wearableType)));
  ...
  /**
   * Get Oura Ring data for a date range
   */
      ...
      return drizzleDb
        ...
        .where(
          and(
            ...
            gte(ouraRingData.date, input.startDate),
            lte(ouraRingData.date, input.endDate)
          )
        )
        .orderBy((t) => t.date);
    }),
  ...
  /**
   * Get Whoop data for a date range
   */
      ...
      return drizzleDb
        ...
        .where(
          and(
            ...
            gte(whoopData.date, input.startDate),
            lte(whoopData.date, input.endDate)
          )
        )
        .orderBy((t) => t.date);
    }),
  ...
  /**
   * Get wearable insights for a date range
   */
  getInsights: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return drizzleDb
        ...
        .where(
          and(
            eq(wearableInsights.userId, ctx.user.id),
            gte(wearableInsights.date, input.startDate),
            lte(wearableInsights.date, input.endDate)
          )
        )
        .orderBy((t) => t.date);
    }),
  ...
  syncOuraData: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const [connection] = await drizzleDb
      .select()
      .from(wearableConnections)
      .where(and(eq(wearableConnections.userId, ctx.user.id), eq(wearableConnections.wearableType, "oura")))
      .limit(1);

    if (!connection) {
      throw new TRPCError({ code: "NOT_FOUND", message: "Oura Ring not connected" });
    }

    // Fetch data from Oura API (last 30 days)
    ...
    if (!response.ok) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch Oura data" });
    }

    const data = await response.json();

    // Store data (simplified - in production, parse all fields)
    for (const day of data.data || []) {
      await drizzleDb
        ...
        .values({
          ...
          deepSleep: day.deep_sleep ? Math.round(day.deep_sleep / 60) : null,
    ...
    return { success: true, message: "Oura data synced successfully", recordsCount: (data.data || []).length };
  }),
  ...
  syncWhoopData: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const [connection] = await drizzleDb
      .select()
      .from(wearableConnections)
      .where(and(eq(wearableConnections.userId, ctx.user.id), eq(wearableConnections.wearableType, "whoop")))
      .limit(1);

    if (!connection) {
      throw new TRPCError({ code: "NOT_FOUND", message: "Whoop not connected" });
    }

    // Fetch data from Whoop API (last 30 days)
    const endDate = new Date();
    const startDate = new Date(endDate.getTime() - 30 * 24 * 60 * 60 * 1000);

    const response = await fetch(
      `https://api.prod.whoop.com/developer/v1/cycle/collection?start=${startDate.toISOString()}&end=${endDate.toISOString()}`,
      {
        headers: { Authorization: `Bearer ${connection.accessToken}` },
      }
    );
    ...
    if (!response.ok) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch Whoop data" });
    }
    ...
    for (const cycle of data.records || []) {
      ...
      await drizzleDb
        ...
        .values({
          ...
          strainScore: Math.round((cycle.strain?.score || 0) * 100 / 21),
          recovery: cycle.recovery?.score,
          recoveryScore: Math.round((cycle.recovery?.score || 0) * 100),
          sleepDuration: cycle.sleep?.total_sleep_duration ? Math.round(cycle.sleep.total_sleep_duration / 60) : null,
          sleepScore: cycle.sleep?.sleep_score,
    ...
    return { success: true, message: "Whoop data synced successfully", recordsCount: (data.records || []).length };
  }),

  /**
   * Get sync status for all wearables
   */
